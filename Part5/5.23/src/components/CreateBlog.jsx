...
export default function CreateBlog({ onCreate, showNotification }) {
    ...
    return (
        <div>
            {
                !showForm ?
                    <button id='create-blog-toggle' onClick={() => setShowForm(true)}>Create New Blog</button>
                    :
                    <>
                        <h2>Create Blog</h2>
                        <form onSubmit={create}>
                            <div>
                                Title: <input id='title' name={formfields.title} required disabled={loading} placeholder='Title'/>
                            </div>
                            <div>
                                URL: <input id='url' name={formfields.url} required disabled={loading} placeholder='URL'/>
                            </div>
                            {
                                loading ?
                                    <p>...Creating, please wait</p>
                                    :
                                    <input id='create-blog-btn' type='submit' value="Create" />
                            }
