...
export default function AnecdoteList() {
    ...
    const handleVote = (anecdote) => {
        ...
        .then((response) => {
            setNotification({ message: `Successfully voted ${anecdote.content}`, status: StatusTypes.SUCCESS, timeout: 2000 })
            console.log(response)
            refetch()
        })
        .catch(() => {
            setNotification({ message: `An error occurred while updating the votes`, status: StatusTypes.ERROR })
        })
    }
